...
int main() {
    ...
    for (size_t i = 0; i < compactProc.size(); i++) {
        if (compactProc[i].empty()) cout << "|    ";
        else cout << "| " << compactProc[i] << " ";
    }
    cout << "|\n";
    ...
    cout << setw(10) << left << "Process"
         ...
         << setw(10) << left << "BT"
         << setw(10) << left << "TAT"
         << setw(10) << left << "WT" << endl;

    for (int i = 0; i < n; i++) {
        cout << setw(10) << left << "P" + processes[i].name
             << setw(10) << left << processes[i].arrival
             << setw(10) << left << processes[i].burst
             << setw(10) << left << processes[i].turnaround
             << setw(10) << left << processes[i].waiting << endl;
    }
}